...
mass_to_kg = 1.989e+30
radius_to_m = 6.957e+8

data['Mass'] = pd.to_numeric(data['Mass']) * mass_to_kg
data['Radius'] = pd.to_numeric(data['Radius']) * radius_to_m
gravity = finalData_df

gravity['Mass'] = pd.to_numeric(data['Mass'])
gravity['Radius'] = pd.to_numeric(data['Radius'])

gravity['Gravity'] = (6.67430e-11 * data['Mass'] * (1.98855e30)) / (data['Radius'] ** 2)
